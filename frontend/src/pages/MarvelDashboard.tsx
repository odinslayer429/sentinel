...
const zToHeat = (z: number) => {
  ...
  if (z > .5)return { label: 'NORMAL',        sub: 'Within expected range',                      color: '#00FFFF' };
  return             { label: 'QUIET',         sub: 'Below average activity',                     color: '#5AC8FA' };
};
const intensityToUrgency = (i: number, c: number) => {
  if (i > 2.5) return { label: 'EXPECT SURGE',     detail: `${c} incidents yesterday — high chance of more in next 3 hrs`, color: '#FF3B30' };
  if (i > 1.5) return { label: 'LIKELY ACTIVE',    detail: `${c} incidents yesterday — above average activity expected`,   color: '#FF9500' };
  if (i > 0.8) return { label: 'WATCH THIS ZONE',  detail: `${c} incidents yesterday — monitor closely`,                  color: '#D2FF00' };
  return             { label: 'CALM',               detail: `${c} incidents yesterday — no surge expected`,                color: '#34C759' };
};
const anomalyToNote = (severity: string, latestCount: number, meanDaily: number) => {
  const excess = Math.max(0, latestCount - Math.round(meanDaily));
  const s = (severity || '').toUpperCase();
  if (s === 'CRITICAL' || s === 'HIGH') return { headline: 'UNUSUAL SPIKE DETECTED', detail: `${latestCount} crimes today vs usual ${Math.round(meanDaily)}/day — ${excess} extra incidents`, color: '#FF3B30' };
  if (s === 'MEDIUM')                   return { headline: 'ABOVE AVERAGE DAY',       detail: `${latestCount} crimes today vs usual ${Math.round(meanDaily)}/day`,                          color: '#FF9500' };
...
const hubTile = { hidden:{opacity:0,y:28,scale:0.97}, show:{opacity:1,y:0,scale:1,transition:{duration:0.45,ease:[0.22,1,0.36,1]}} };
const pageVariants = { initial:{opacity:0,y:18}, animate:{opacity:1,y:0,transition:{duration:0.35,ease:[0.22,1,0.36,1]}}, exit:{opacity:0,y:-10,transition:{duration:0.2}} };
...
// ─── IntelBrick ───────────────────────────────────────────────────────────────
function IntelBrick({zone,zoneName,headline,detail,action,crimeType,extra,color,index}:
  {zone:string;zoneName?:string;headline:string;detail:string;action?:string;crimeType?:string;extra?:string;color:string;index:number}) {
  return (
    <motion.div initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:index*0.05}}
      className="tactical-card" style={{padding:'1.5rem',borderLeft:`3px solid ${color}`,display:'flex',flexDirection:'column',gap:'0.6rem'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        {action&&(
          <div style={{textAlign:'right'}}>
            ...
            <div style={{fontSize:'0.65rem',fontWeight:900,color,letterSpacing:1,padding:'4px 10px',border:`1px solid ${color}44`,background:`${color}11`}}>{action}</div>
          </div>
        )}
      </div>
      ...
      {crimeType&&<div style={{display:'inline-block',padding:'3px 10px',fontSize:'0.55rem',letterSpacing:1,border:'1px solid rgba(210,255,0,0.2)',color:'#D2FF00',background:'rgba(210,255,0,0.06)',alignSelf:'flex-start'}}>{crimeType.replace(/_/g,' ')}</div>}
        ...
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          ...
          <tbody>{schedule.map((s,i)=>(
            <tr key={i} style={{borderTop:'1px solid rgba(255,255,255,0.04)'}}>
              ...
              <td style={{padding:'1rem',fontWeight:900,color:s.mult>1?'#FF9500':'#34C759'}}>{s.mult>1?'DOUBLE STRENGTH':'STANDARD STRENGTH'}</td>
            </tr>
          ))}</tbody>
        </table>
      )}
...
function AIIntakeSection() {
  ...
  const analyze=async()=>{
    ...
    try{
      let res;
      if(file){const fd=new FormData();fd.append('file',file);res=await axios.post('/api/fir/analyze-pdf',fd,{headers:{'Content-Type':'multipart/form-data'}});}
      else{res=await axios.post('/api/fir/analyze',{text:complaint});}
      setResult(res.data);
...
function NeuralNodePanel() {
  ...
  const fetchData=useCallback(async()=>{
    ...
    }catch(e:any){setError(e?.response?.data?.detail||'Failed to load predictions.');}
    finally{setLoading(false);}
  },[]);
    ...
    <section>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1px',background:'rgba(255,255,255,0.04)',marginBottom:'2rem'}}>
        {[{label:'ZONES TO DEPLOY TO NOW',val:criticalCount,color:'#FF3B30',note:'These zones need officers right now'},
          {label:'ZONES WITH UNUSUAL SPIKES',val:anomalies.length,color:'#FF9500',note:'More crimes than normal today'},
      ...
      {hotspots.length>0&&(
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'1px',background:'rgba(255,255,255,0.04)',marginBottom:'2rem'}}>
          {hotspots.map((h,i)=>{const urg=intensityToUrgency(h.predicted_intensity,h.crimes_last_24h);return<IntelBrick key={i} index={i} zone={h.zone_id} headline={urg.label} detail={urg.detail} action={riskToAction(h.risk_level)} crimeType={h.top_crime_type} color={urg.color}/>;})}
        </div></>
      )}
      {anomalies.length>0&&(
        <><div className="section-label" style={{marginBottom:'1rem'}}>ZONES WITH UNUSUAL ACTIVITY TODAY</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'1px',background:'rgba(255,255,255,0.04)',marginBottom:'2rem'}}>
          {anomalies.map((a,i)=>{const note=anomalyToNote(a.severity,a.latest_count,a.mean_daily);return<IntelBrick key={i} index={i} zone={a.zone_id} headline={note.headline} detail={note.detail} action={riskToAction(a.severity)} color={note.color} extra={`30-DAY AVERAGE: ${Math.round(a.mean_daily)} crimes/day`}/>;})}
        </div></>
      )}
      {forecast.length>0&&(
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'1px',background:'rgba(255,255,255,0.04)'}}>
          {forecast.map((f,i)=>{const pl=f.peak_risk==='CRITICAL'?'PEAK DANGER EXPECTED':f.peak_risk==='HIGH'?'HIGH ACTIVITY EXPECTED':'MODERATE ACTIVITY';const pc=f.peak_risk==='CRITICAL'?'#FF3B30':f.peak_risk==='HIGH'?'#FF9500':'#D2FF00';return<IntelBrick key={i} index={i} zone={f.zone_id} headline={pl} detail={`Peak activity expected in ${f.peak_hour_offset} hour${f.peak_hour_offset!==1?'s':''}. ${f.crime_count_24h} incidents in last 24 hrs.`} action={riskToAction(f.peak_risk)} color={pc} extra="FORECAST WINDOW: 6 HOURS"/>;})}
        </div></>
      )}
    </section>
  );
  ...
  return(
    <section>
      ...
      {surging.length>0&&(
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'1px',background:'rgba(255,255,255,0.04)',marginBottom:'2rem'}}>
          {surging.map((z,i)=>{const h=zToHeat(z.z_score);return<IntelBrick key={z.zone_id} index={i} zone={z.zone_id} zoneName={z.zone_name} headline={h.label} detail={`${z.current_1h} incidents this hour vs usual ${z.mean_1h?.toFixed(0)} per hour. ${h.sub}.`} action="DEPLOY IMMEDIATELY" color={h.color}/>;})}
        </div></>
      )}
      {elevated.length>0&&(
        <><div className="section-label" style={{color:'#FF9500',marginBottom:'1rem'}}>↑ ABOVE NORMAL — INCREASE PATROLS</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'1px',background:'rgba(255,255,255,0.04)',marginBottom:'2rem'}}>
          {elevated.map((z,i)=>{const h=zToHeat(z.z_score);return<IntelBrick key={z.zone_id} index={i} zone={z.zone_id} zoneName={z.zone_name} headline={h.label} detail={`${z.current_1h} incidents this hour vs usual ${z.mean_1h?.toFixed(0)} per hour.`} action="INCREASE PATROLS" color={h.color}/>;})}
        </div></>
      )}
      {normal.length>0&&(
  ...
  const scan=async()=>{
    ...
    try{
      ...
      if(backendErr.response?.status===404||backendErr.response?.status===405){
        const flags:string[]=[];let score=50;
        if(scanType==='URL'){
      ...
      }else{setError(backendErr?.response?.data?.detail||'Scan failed.');}
    }finally{setLoading(false);}
  };
      ...
      <div className="tactical-card">
        ...
        <div style={{display:'flex',gap:'0.5rem',marginBottom:'1rem'}}>
          {(['URL','PHONE','NAME'] as const).map(t=>(
            <button key={t} onClick={()=>setScanType(t)} style={{padding:'0.5rem 1rem',border:'1px solid '+(scanType===t?'#D2FF00':'rgba(255,255,255,0.1)'),background:scanType===t?'rgba(210,255,0,0.1)':'transparent',color:scanType===t?'#D2FF00':'#555',fontFamily:'Space Mono,monospace',fontSize:'0.6rem',cursor:'pointer',letterSpacing:2}}>
              {t==='URL'?'WEBSITE LINK':t==='PHONE'?'PHONE NUMBER':'PERSON NAME'}
            </button>
          ))}
        </div>
        <div style={{display:'flex',gap:'1rem'}}>
          ...
          <button onClick={scan} disabled={!target.trim()||loading} className="back-btn"
            style={{position:'relative',top:0,left:0,minWidth:120,fontWeight:900}}>{loading?'CHECKING...':'CHECK NOW'}</button>
        </div>
      </div>
      {loading&&<Spinner/>}
      ...
      {result&&!loading&&(
        <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}
          ...
          {result.source==='HEURISTIC'&&(
            <div style={{fontSize:'0.55rem',color:'#FF9500',letterSpacing:2,marginBottom:'1rem',padding:'0.5rem',background:'rgba(255,149,0,0.08)',border:'1px solid rgba(255,149,0,0.2)'}}>⚠ BASIC CHECK ONLY — Backend scanner not connected.</div>
          )}
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}>
            <div><div style={{fontSize:'0.55rem',color:'#555',letterSpacing:2}}>VERDICT</div><div style={{fontWeight:900,fontSize:'1.4rem',color:verdictColor(result.verdict),letterSpacing:2,marginTop:4}}>{result.verdict}</div></div>
            <div style={{textAlign:'right'}}><div style={{fontSize:'0.55rem',color:'#555',letterSpacing:2}}>RISK SCORE</div><div style={{fontSize:'3rem',fontWeight:900,color:verdictColor(result.verdict),lineHeight:1}}>{result.trust_score}</div><div style={{fontSize:'0.5rem',color:'#444'}}>out of 100</div></div>
          </div>
          ...
          {result.flags?.length>0&&(
            <div><div style={{fontSize:'0.55rem',color:'#FF3B30',fontWeight:900,letterSpacing:2,marginBottom:'0.5rem'}}>PROBLEMS FOUND</div>
              {result.flags.map((f:string,i:number)=>(<div key={i} style={{padding:'0.6rem 1rem',marginBottom:'0.5rem',background:'rgba(255,59,48,0.06)',border:'1px solid rgba(255,59,48,0.2)',color:'#FF3B30',fontSize:'0.65rem',letterSpacing:1}}>⚠ {f}</div>))}
            </div>
          )}
...
export default function MarvelDashboard() {
  ...
  const [wsNewCount,setWsNewCount]=useState(0); // badge: new events arrived via WS
  ...
  const { connected: wsConnected } = useWebSocket({
    onNewEvent: (ev) => {
      // Prepend to events list; bump stats.total_24h
      setEvents(prev => {
        const already = prev.some(e => e.url === ev.url);
        if (already) return prev;
        return [ev as unknown as Event, ...prev].slice(0, 500);
      });
      setStats(prev => prev ? { ...prev, total_24h: prev.total_24h + 1 } : prev);
      setWsNewCount(c => c + 1);
      // Auto-reset badge after 6 s
      ...
      <header className="hud-header">
        ...
        <div style={{display:'flex',gap:'2rem',alignItems:'center'}}>
          ...
          <div className="status-indicator">
            <div style={{width:7,height:7,borderRadius:'50%',background:wsConnected?'#34C759':'#FF3B30',
              boxShadow:wsConnected?'0 0 8px #34C759':'0 0 8px #FF3B30',transition:'all 0.5s'}}/>
            <span style={{fontSize:'0.5rem',letterSpacing:2,color:wsConnected?'#34C759':'#FF3B30'}}>
              {wsConnected?'LIVE PUSH':'POLLING'}
            </span>
          </div>
          {wsNewCount>0&&(
            <motion.div initial={{scale:0}} animate={{scale:1}}
              style={{padding:'3px 10px',fontSize:'0.5rem',letterSpacing:2,background:'rgba(210,255,0,0.12)',
                border:'1px solid rgba(210,255,0,0.4)',color:'#D2FF00'}}>
              +{wsNewCount} NEW
            </motion.div>
          )}
                ...
                <div className="strategic-tray">
                  ...
                  <div className="tray-item">
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:'0.4rem',marginTop:'0.5rem'}}>
                      {[['🔴','Theft / Robbery','#FF3B30'],['🟠','Assault','#FF9500'],['🔵','Cyber Crime','#00FFFF']]
                        .map(([dot,label,color],i)=>(<div key={i} style={{fontSize:'0.6rem',color:color as string}}>{dot} {label}</div>))}
                    </div>
                  </div>
                  <div className="tray-item">
