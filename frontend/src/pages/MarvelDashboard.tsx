...
const zToHeat       = (z:number) => { if(z>5)return{label:'EXTREME SPIKE',sub:'Far above normal',color:'#FF2D55'}; if(z>3)return{label:'HIGH ALERT',sub:'Well above normal — deploy extra units',color:'#FF3B30'}; if(z>2)return{label:'ELEVATED',sub:'Above normal — increase patrols',color:'#FF9500'}; if(z>1)return{label:'ABOVE NORMAL',sub:'Slightly higher than usual',color:'#D2FF00'}; if(z>.5)return{label:'NORMAL',sub:'Within expected range',color:'#00FFFF'}; return{label:'QUIET',sub:'Below average',color:'#5AC8FA'}; };
const intensityToUrgency = (i:number,c:number) => { if(i>2.5)return{label:'EXPECT SURGE',detail:`${c} incidents yesterday — high chance in next 3 hrs`,color:'#FF3B30'}; if(i>1.5)return{label:'LIKELY ACTIVE',detail:`${c} incidents yesterday — above average expected`,color:'#FF9500'}; if(i>0.8)return{label:'WATCH THIS ZONE',detail:`${c} incidents yesterday — monitor closely`,color:'#D2FF00'}; return{label:'CALM',detail:`${c} incidents yesterday — no surge expected`,color:'#34C759'}; };
const anomalyToNote = (severity:string,latestCount:number,meanDaily:number) => { const excess=Math.max(0,latestCount-Math.round(meanDaily)); const s=(severity||'').toUpperCase(); if(s==='CRITICAL'||s==='HIGH')return{headline:'UNUSUAL SPIKE DETECTED',detail:`${latestCount} crimes today vs usual ${Math.round(meanDaily)}/day — ${excess} extra`,color:'#FF3B30'}; if(s==='MEDIUM')return{headline:'ABOVE AVERAGE DAY',detail:`${latestCount} crimes today vs usual ${Math.round(meanDaily)}/day`,color:'#FF9500'}; return{headline:'SLIGHTLY ELEVATED',detail:`${latestCount} crimes today vs usual ${Math.round(meanDaily)}/day`,color:'#D2FF00'}; };
...
function ScrollBar() {
  ...
  useEffect(() => {
    const onScroll = () => { const el=document.documentElement; const max=el.scrollHeight-el.clientHeight; setPct(max>0?(window.scrollY/max)*100:0); };
...
function CountUp({to,duration=1200}:{to:number;duration?:number}) {
  ...
  useEffect(()=>{ if(!to){setVal(0);return;} let cur=0; const step=Math.max(1,Math.ceil(to/(duration/16))); const id=setInterval(()=>{ cur=Math.min(cur+step,to); setVal(cur); if(cur>=to)clearInterval(id); },16); return()=>clearInterval(id); },[to]);
  return <>{val}</>;
}

// ─── Section header ───────────────────────────────────────────────────────────
function SectionHead({index,eyebrow,title,status,statusColor='var(--lime)'}:{index:string;eyebrow:string;title:string;status?:string;statusColor?:string}) {
  return (
    <div style={{marginBottom:'3rem'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:'1rem'}}>
        ...
        {status&&<div style={{marginLeft:'auto',fontFamily:'Space Mono,monospace',fontSize:'0.5rem',letterSpacing:'0.2em',color:statusColor,animation:'pulse 2s ease-in-out infinite'}}>{status}</div>}
      </div>
  ...
  return (
    <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:index*0.05,duration:0.4,ease:[0.22,1,0.36,1]}}
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        {action&&<div style={{textAlign:'right'}}><div style={{fontFamily:'Space Mono,monospace',fontSize:'0.45rem',color:'var(--text-3)',letterSpacing:'0.2em',marginBottom:2}}>ACTION</div><div style={{fontFamily:'Space Mono,monospace',fontSize:'0.6rem',fontWeight:900,color,letterSpacing:'0.1em',padding:'4px 10px',border:`1px solid ${color}44`,background:`${color}11`}}>{action}</div></div>}
      </div>
      ...
      {crimeType&&<div style={{display:'inline-block',padding:'3px 10px',fontFamily:'Space Mono,monospace',fontSize:'0.5rem',letterSpacing:'0.1em',border:'1px solid rgba(210,255,0,0.2)',color:'#D2FF00',background:'rgba(210,255,0,0.05)',alignSelf:'flex-start'}}>{crimeType.replace(/_/g,' ')}</div>}
      ...
      <AnimatePresence>
        {schedule.length>0&&(
          <motion.table initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
            <thead><tr><th>DAY</th><th>SHIFT TYPE</th><th>STAFFING</th></tr></thead>
            <tbody>{schedule.map((s,i)=>(<motion.tr key={i} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}} transition={{delay:i*0.06}}><td style={{fontFamily:'Space Mono,monospace',fontWeight:900,color:'#D2FF00'}}>{s.day.toUpperCase()}</td><td style={{color:'var(--text-2)'}}>{s.shift}</td><td style={{fontFamily:'Space Mono,monospace',fontWeight:900,color:s.mult>1?'#FF9500':'#34C759'}}>{s.mult>1?'DOUBLE STRENGTH':'STANDARD'}</td></motion.tr>))}</tbody>
          </motion.table>
        )}
      </AnimatePresence>
    </div>
    ...
    <div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem',marginBottom:'1rem'}}>
        <div onClick={()=>fileInputRef.current?.click()} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={e=>{e.preventDefault();setDragOver(false);if(e.dataTransfer.files?.[0]){setFile(e.dataTransfer.files[0]);setComplaint('');}}} style={{border:`2px dashed ${dragOver?'var(--lime)':file?'rgba(210,255,0,0.4)':'rgba(255,255,255,0.1)'}`,padding:'2rem',textAlign:'center',cursor:'pointer',background:dragOver?'rgba(210,255,0,0.06)':file?'rgba(210,255,0,0.03)':'transparent',transition:'all 0.2s',boxShadow:dragOver?'0 0 20px rgba(210,255,0,0.15)':'none'}}>
          ...
          {file&&<button onClick={e=>{e.stopPropagation();clearFile();}} style={{background:'none',border:'none',color:'#FF3B30',fontSize:'0.6rem',marginTop:'0.5rem',cursor:'pointer',fontFamily:'Space Mono,monospace'}}>REMOVE ✕</button>}
        </div>
...
function NeuralNodePanel() {
  ...
  return(
    <div>
      ...
      {forecast.length>0&&(<><div className="section-label">6-HOUR OUTLOOK</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'1px',background:'var(--border)'}}>{forecast.map((f,i)=>{const pl=f.peak_risk==='CRITICAL'?'PEAK DANGER':f.peak_risk==='HIGH'?'HIGH ACTIVITY':'MODERATE';const pc=f.peak_risk==='CRITICAL'?'#FF3B30':f.peak_risk==='HIGH'?'#FF9500':'#D2FF00';return<IntelBrick key={i} index={i} zone={f.zone_id} headline={pl} detail={`Peak in ${f.peak_hour_offset}h. ${f.crime_count_24h} incidents last 24h.`} action={riskToAction(f.peak_risk)} color={pc} extra="WINDOW: 6 HRS"/>;})}</div></>)}
    </div>
  );
}
...
function OffenderSection({offenders}:{offenders:Offender[]}) {
  ...
  return(
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1px',background:'var(--border)',marginBottom:'2.5rem'}}>{[{label:'TOTAL',val:offenders.length,color:'#D2FF00'},{label:'HIGH RISK',val:offenders.filter(o=>o.predicted_risk==='High').length,color:'#FF3B30'},{label:'MEDIUM',val:offenders.filter(o=>o.predicted_risk==='Medium').length,color:'#FF9500'},{label:'AVG FIRs',val:offenders.length?(offenders.reduce((a,b)=>a+b.fir_count,0)/offenders.length).toFixed(1):'0',color:'#00E5FF'}].map((s,i)=>(<div key={i} className="tactical-card" style={{textAlign:'center',padding:'1.75rem'}}><div style={{fontFamily:'Space Mono,monospace',fontSize:'2.2rem',fontWeight:900,color:s.color}}>{s.val}</div><div className="card-label" style={{marginTop:'0.5rem'}}>{s.label}</div></div>))}</div>
      <div style={{display:'flex',gap:'0.75rem',marginBottom:'1.5rem',alignItems:'center',flexWrap:'wrap'}}><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="SEARCH BY NAME OR ALIAS..." style={{flex:1,minWidth:200,padding:'0.75rem 1rem',fontSize:'0.7rem'}}/>{['ALL','High','Medium','Low'].map(r=>(<button key={r} onClick={()=>setFilter(r)} style={{padding:'0.65rem 1rem',border:`1px solid ${filter===r?'var(--lime)':'rgba(255,255,255,0.1)'}`,background:filter===r?'rgba(210,255,0,0.08)':'transparent',color:filter===r?'#D2FF00':'var(--text-3)',fontFamily:'Space Mono,monospace',fontSize:'0.55rem',cursor:'pointer',transition:'all 0.2s'}}>{r==='ALL'?'ALL':r.toUpperCase()+' RISK'}</button>))}</div>
      <div style={{display:'grid',gridTemplateColumns:selected?'1fr 380px':'1fr',gap:'1px',background:'var(--border)',alignItems:'start'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:'1px',background:'var(--border)'}}>
          {filtered.map((off,i)=>(<motion.div key={i} className="tactical-card" initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:Math.min(i*0.04,0.5)}} onClick={()=>setSelected(selected?.name===off.name?null:off)} style={{cursor:'pointer',borderLeft:`3px solid ${riskColor(off.predicted_risk)}`,outline:selected?.name===off.name?'1px solid rgba(210,255,0,0.25)':undefined}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1rem'}}><div><div style={{fontFamily:'Space Mono,monospace',fontWeight:900,fontSize:'0.85rem',letterSpacing:'0.1em'}}>{off.name}</div>{off.alias&&<div style={{fontSize:'0.5rem',color:'var(--text-3)',marginTop:2}}>AKA: {off.alias}</div>}</div><span style={{padding:'3px 8px',fontFamily:'Space Mono,monospace',fontSize:'0.5rem',fontWeight:900,background:riskBg(off.predicted_risk),color:riskColor(off.predicted_risk),border:`1px solid ${riskColor(off.predicted_risk)}44`}}>{off.predicted_risk==='High'?'HIGH':off.predicted_risk==='Medium'?'MED':'LOW'}</span></div><div style={{display:'flex',gap:'1.5rem',marginBottom:'0.75rem'}}><div><div className="card-label">FIRs</div><div style={{fontFamily:'Space Mono,monospace',fontWeight:900,color:off.fir_count>3?'#FF3B30':'var(--text-1)'}}>{off.fir_count}</div></div><div><div className="card-label">LAST SEEN</div><div style={{fontSize:'0.65rem',fontWeight:700,color:'var(--text-2)'}}>{off.last_seen?new Date(off.last_seen).toLocaleDateString('en-IN'):'UNKNOWN'}</div></div><div><div className="card-label">ZONE</div><div style={{fontFamily:'Space Mono,monospace',fontSize:'0.65rem',fontWeight:700}}>{Array.isArray(off.zones)?off.zones[0]:(off.zones||'?')}</div></div></div>{off.recidivism_probability!==undefined&&<div><div style={{height:'2px',background:'rgba(255,255,255,0.06)',borderRadius:1}}><motion.div initial={{width:0}} animate={{width:Math.round(off.recidivism_probability*100)+'%'}} transition={{duration:0.8,ease:[0.22,1,0.36,1]}} style={{height:'100%',background:riskColor(off.predicted_risk),borderRadius:1}}/></div></div>}</motion.div>))}
          {filtered.length===0&&<div style={{gridColumn:'1/-1'}}><EmptyState icon="👤" msg="NO MATCHES"/></div>}
        </div>
        <AnimatePresence>{selected&&(<motion.div initial={{opacity:0,x:24}} animate={{opacity:1,x:0}} exit={{opacity:0,x:24}} transition={{duration:0.35,ease:[0.22,1,0.36,1]}} className="tactical-card" style={{position:'sticky',top:'80px',borderColor:'rgba(210,255,0,0.15)'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'2rem'}}><div className="section-label" style={{margin:0,border:'none',padding:0}}>PROFILE</div><button onClick={()=>setSelected(null)} style={{background:'none',border:'1px solid rgba(255,255,255,0.1)',padding:'4px 10px',cursor:'pointer',fontFamily:'Space Mono,monospace',fontSize:'0.6rem',color:'var(--text-3)',transition:'all 0.2s'}}>✕</button></div><div style={{fontFamily:'Space Mono,monospace',fontSize:'1rem',fontWeight:900,letterSpacing:'0.1em',color:'var(--text-1)',marginBottom:'0.25rem'}}>{selected.name}</div>{selected.alias&&<div style={{fontSize:'0.55rem',color:'var(--text-3)',marginBottom:'1.5rem'}}>AKA: {selected.alias}</div>}{[{label:'THREAT',value:selected.predicted_risk||'UNKNOWN',color:riskColor(selected.predicted_risk)},{label:'FIRs',value:String(selected.fir_count)},{label:'LAST SEEN',value:selected.last_seen?new Date(selected.last_seen).toLocaleDateString('en-IN'):'UNKNOWN'},{label:'AREAS',value:Array.isArray(selected.zones)?selected.zones.join(', '):(selected.zones||'?')}].map((row,i)=>(<div key={i} style={{padding:'0.75rem 0',borderBottom:'1px solid rgba(255,255,255,0.04)'}}><div className="card-label">{row.label}</div><div style={{fontWeight:700,fontSize:'0.85rem',marginTop:4,color:row.color||'var(--text-1)'}}>{row.value}</div></div>))}{selected.recidivism_probability!==undefined&&<div style={{marginTop:'1rem'}}><div className="card-label">RE-OFFEND PROBABILITY</div><div style={{display:'flex',alignItems:'center',gap:'1rem',marginTop:8}}><div style={{flex:1,height:'4px',background:'rgba(255,255,255,0.06)',borderRadius:2}}><motion.div initial={{width:0}} animate={{width:Math.round(selected.recidivism_probability*100)+'%'}} transition={{duration:1}} style={{height:'100%',background:riskColor(selected.predicted_risk),borderRadius:2}}/></div><span style={{fontFamily:'Space Mono,monospace',fontSize:'0.75rem',fontWeight:900,color:riskColor(selected.predicted_risk)}}>{Math.round(selected.recidivism_probability*100)}%</span></div></div>}</motion.div>)}</AnimatePresence>
      </div>
    </div>
  );
}
...
export default function MarvelDashboard() {
    ...
    try {
      ...
      if (s.status === 'fulfilled') {
        ...
        setStats({
          ...
          warning:   d.warning   ?? d.crimes_this_week   ?? 0,
        });
      }
      if (v.status === 'fulfilled') setVelocity(Array.isArray(v.value.data) ? v.value.data : v.value.data.zones || []);
      if (a.status === 'fulfilled') {
        const ad = a.value.data;
        const arr = Array.isArray(ad) ? ad : ad.alerts || [];
        setAlerts(arr.map((x: any) => ({
          zone:     x.zone    || x.zone_id  || '',
          message:  x.message || x.crime_type || '',
          severity: x.severity?.toString() || '',
        })));
      }
      if (e.status === 'fulfilled') {
        const raw = e.value.data;
        const evs = (Array.isArray(raw) ? raw : raw.events || raw.items || []) as any[];
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* FOOTER */}
      <div style={{padding:'2rem 3rem',borderTop:'1px solid var(--border)',display:'flex',justifyContent:'space-between',flexWrap:'wrap',gap:'1rem'}}>
        <span style={{fontFamily:'Space Mono,monospace',fontSize:'0.45rem',letterSpacing:'0.2em',color:'var(--text-3)'}}>SENTINEL · MAHARASHTRA POLICE AI</span>
        <span style={{fontFamily:'Space Mono,monospace',fontSize:'0.45rem',letterSpacing:'0.2em',color:'var(--text-3)'}}>CLASSIFICATION: RESTRICTED</span>
        <span style={{fontFamily:'Space Mono,monospace',fontSize:'0.45rem',letterSpacing:'0.2em',color:'var(--text-3)'}}>© 2026 SENTINEL SYSTEMS</span>
      </div>

      {/* TICKER */}
      <div className="live-ticker-wrap">
        <div style={{display:'flex',animation:'ticker 40s linear infinite',gap:'4rem',whiteSpace:'nowrap'}}>
          {alerts.length>0?alerts.map((a,i)=>(<span key={i} style={{color:a.severity==='CRITICAL'?'var(--red)':a.severity==='HIGH'?'var(--orange)':'var(--lime)'}}>▸ {a.zone} : {a.message}</span>)):<span style={{color:'var(--text-3)'}}>SENTINEL v4.0 · ALL SYSTEMS NOMINAL · {new Date().toLocaleDateString('en-IN')}</span>}
          <span style={{color:'var(--text-3)'}}>· LIVE · {new Date().toLocaleDateString('en-IN')}</span>
        </div>
      </div>
      ...
      <AnimatePresence>
        {surges.length>0&&(
          <motion.div initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:20}} style={{position:'fixed',top:'80px',right:'2rem',width:'260px',pointerEvents:'none',zIndex:999}}>
            <div style={{fontFamily:'Space Mono,monospace',fontSize:'0.5rem',letterSpacing:'0.3em',color:'var(--red)',marginBottom:'0.5rem'}}>⚠ CLUSTERING NOW</div>
            {surges.slice(0,3).map((s,i)=>(<motion.div key={i} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:i*0.1}} style={{pointerEvents:'auto',marginBottom:'2px',padding:'0.75rem 1rem',border:'1px solid rgba(255,59,48,0.25)',background:'rgba(255,59,48,0.05)'}}><div style={{fontFamily:'Space Mono,monospace',fontWeight:900,fontSize:'0.75rem',color:'var(--red)'}}>{s.zone}</div><div style={{fontSize:'0.55rem',color:'var(--text-2)',marginTop:2}}>{s.message}</div></motion.div>))}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
